...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(empty($error_msg)){
        ...
        $stmt = $con->prepare('INSERT INTO android_course (`fullName`, `email`, `phone`, `university`, `department`, `status`, `about`) VALUES (?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute(array($name, $email,  $phone, $university, $department, $status, $about ));

    }else{
        $_SESSION['errors'] = $error_msg;
        $_SESSION['data'] = $data;
        header('Location: android Registration Form.php');
        exit();
    }
